...
export function createCubeAtCursor(event: MouseEvent, camera: THREE.PerspectiveCamera, scene: THREE.Scene, renderer: THREE.WebGLRenderer) {
  ...
  animateCube(cube, cubeDestinationPosition, scene, camera, renderer);
}
...
function animateCube(cube: THREE.Mesh, cubeDestinationPosition: THREE.Vector3, scene: THREE.Scene, camera: THREE.PerspectiveCamera, renderer: THREE.WebGLRenderer) {
